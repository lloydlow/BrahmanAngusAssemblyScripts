...
path2 <- paste0(dir1,"bostaurus_brahma_bionano_NCBI_full_corrected_gapfill_arrow_fil_withM_No_Ns.rls")
...
path3 <- paste0(dir1,"bostaurus_angus_bionano_NCBI_full_corrected_gapfill_arrow_fil_No_Ns.rls")
...
#change X to X/Y
df$chromosome <- gsub("X","X/Y",df$chromosome)

#plot barplot
df$chromosome <- factor(df$chromosome, levels = df$chromosome[1:30])

tiff(filename = "FigFinal_Brahman_angus_vs_dominette_length_comparison.tiff",width = 600, height = 600)
